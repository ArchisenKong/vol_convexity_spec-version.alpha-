# entity/input_schema.md · T3-18 funding relation failure · v1.2 · 20260810

> **v1.2（20260810，清账序3+4出版批·自然再版，档三C-8，KD裁定＝裁决84；cp-then-edit自v1.1，底版md5＝`81b9e917c8678f8c69938850dbdc6316`）**：①**文件名补「一阶实体_」前缀**（DOC-17(a)兑现，60-D-f容器；原文件名`T3-18_input_schema_v1_1_20260726.md`保留为历史件）；②§2随行指针扩第③条 roll_cost→TPL3-T318-02（DOC-03兑现）＋第②条量甲/量乙随行注（83-4指针）。schema实质内容零改动、零判定翻转（增量复核项2范围符合性不受触碰——本再版仅指针面与文件名面）。
...
## §2 随行指针三条（v1.2扩；翻译审计表v1.1 T3·609行，非漂移，消费方须随行携带，本根不就地裁口径）

1. **premium口径与宪-ST-06时间价值成分口径之同源性** → 治理册槽位候选信号#7（Phase B定）。本根不做同源性核验，随行携带指针。
2. **bleed端口径** → TPL3-T24-01（open，D6登记，任务6产出v1.7类三项挂A→B衔接清单）。〔v1.2随行注：量甲/量乙职能二分见裁决83-4；本根消费之`bleed_cents`为人造账本值，消费面声明义务（F-19）自第五批任务包起前瞻适用，本根存量不回溯〕
3. **roll_cost口径** → **TPL3-T318-02**（裁决31-M登记；另指治理册信号#13）。〔v1.2补，DOC-03兑现：该开放项系schema v1.1定稿后登记，本条补齐三成分指针对称性；下游消费方读本schema即见〕本根消费bleed为账本数值（entity/ledger_input.json `bleed_cents` 字段），数据/机制刀口下值消费不登记；bleed本身如何由定价/重估得出（T-24 §4①②之类一/类二处置）不在本根范围，引用不重登（一阶实体_T-24_input_schema.md §4-§7先例）。
