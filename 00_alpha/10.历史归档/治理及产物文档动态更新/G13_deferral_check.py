#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""G13_deferral_check.py · v1.2 · 20260810（左边界断言＋T系两位下限：QT-002/FW-04-甲子串误报、四2转化候选短码碰撞三类误报订正）
...
# 递延段落识别锚（段标题级，宽匹配）
DEFER_HEADS = ['落地去向', '递延项', '显式后置', '续记', '下游动作序', '后置池', '排程']
...
def main():
    ...
    for f in sorted(glob.glob(os.path.join(doc_dir,'裁决*落地文档*.md'))):
        total_docs+=1
