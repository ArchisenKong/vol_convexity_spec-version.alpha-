...
> **v2.2修订（20260714，W-7扫描H1裁定落地）**：§3 bucket阈值取值补三分定性注记（同GK-14 §2.2型：函数结构＝实体算法要件已定〔裁决14〕；阈值取值＝类三阶段B校准候选，声明值不外推，不登记模板一/二）。算法本体与全部数值零变动，A/B产物零变动。
...
> **阈值取值三分定性〔v2.2，W-7扫描H1裁定〕**：bucket函数结构（分段映射形态）＝实体算法要件，已定（裁决14）；三组阈值取值（tenor 30/60/90/180天、moneyness 0.95/1.05、liquidity 100/1000）＝**类三阶段B校准候选**——本根声明值，待阶段B实盘/回测数据证伪，不外推，不登记模板一/二（同型先例：GK-14 §2.2 w_τ"结构类一已定、数值类三"）。历史注记：T-76首打时期登记编号M1-T76-01（bucket边界参数缺数值）之语义由本注记承接（见W-7扫描记录OBS-1）。
